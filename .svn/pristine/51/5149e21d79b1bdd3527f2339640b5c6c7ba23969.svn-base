         ...
         <div class="editable-table">
           <table cellspacing="0" cellpadding="0">
             ...
             <tr>
               ...
               <td>
                 <Input type="number" v-model="list.quantity" style="width:100px"></Input>
               </td>
               <td class="col-cz">
                 {{list.batchId}}
               </td>
           ...
           <table cellspacing="0" cellpadding="0">
             ...
             <tr>
               ...
               <template v-for="(item,index) in list.itemOneObj" >
                 <td><Input  type="number" v-model="item.standard.length_size" /></td>
               </template>
             </tr>
             <tr>
               <th>电阻</th>
               <template v-for="(item,index) in list.itemOneObj" >
                 <td><Input  type="number" v-model="item.standard.resistance" /></td>
               </template>
             </tr>
             </thead>
             <tbody>
...
<script>
  ...
  export default {
    ...
    methods: {
      load() {
        this.loading = 1;
        this.$http.post("/api/revision/getNewCheckElectrode?batchId=" + this.batchId+"&versionId="+this.versionId,{}).then((res) => {
          this.loading = 0;
          if (res.data.code == 0) {
            if(res.data.data){
      ...
      save(){
        ...
        form.signDate = page.formatDate(form.signDate);
          form.inspectionItem=JSON.stringify(form.itemOneObj);
        console.log(form)
        /*// 表头信息
        ...
        // 提交
        this.loading = 1;
        var uri = '/api/revision/addAllCheckElectrode';
        /*if(this.pageFlag == 2){
