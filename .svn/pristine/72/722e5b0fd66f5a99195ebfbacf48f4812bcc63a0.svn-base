<template>
<Modal v-model="display" title="选择入库申请单" :closable="false" :mask-closable="false" width="800" class="selinstockaply" :transfer="transfer">
  <div class="page">
    <div class="page-searchbox">
      <table cellpadding="0" cellspacing="0">
        <tr>
          <td>
            <Input v-model="queryForm.inStockApplyId" placeholder="申请单号" ></Input>
          </td> 
          <td>
...
<script>
  ...
  import SelAudit from './SelAudit';
  import page from '@/assets/js/page';
  export default {
    components: {
      Loading,Editable,SelAudit
    },
    props:{
      transfer:{
        type:Boolean,
    ...
    methods: {
      load() {
        ...
        this.queryParam.storageId = this.options.storageId;
        this.queryParam.status = 1;
        this.$http.post("/api/inStockApply/list", this.queryParam).then((res) => {
          this.loading = 0;
          if (res.data.code === 0) {
            this.loading = 0;
      ...
      loadDetails(id) { 
        this.loading = 1;
        this.$http.post("/api/inStockApply/get?inStockApplyId=" + id).then((res) => {
          this.loading = 0;
          if (res.data.code === 0) {
            this.loading = 0;
      ...
      innerCheckRow(index){
        ...
        this.loadDetails(this.list[index].inStockApplyId);
      },
      reset: function () {
      ...
      reject(){
        ...
        if(select == null){
         ...
         this.$refs.audit.open({
          ok:(opt)=>{
            this.reject(opt.auditRemark);
            this.$http.post('/api/inStockApply/reject', {
            inStockApplyId: select.inStockApplyId,
            auditRemark:opt.auditRemark,
            }).then((res) => {
              /*执行成功*/
              if (res.data.code === 0) {
                this.query();
