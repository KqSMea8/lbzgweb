  ...
  <Editable @add="add" @remove="remove" :editable="editable">
    ...
    <table cellspacing="0" cellpadding="0" v-else>
        <thead>
          ...
          <th >供应商</th>
          <th >件数</th>
          <th class="col-w2"><span>车号</span></th>
          <th >毛种</th>
          <th >扣杂</th>
          <th >皮重</th>
          <th class="col-quantity"><span>净重</span></th> 
          <th >单位</th>
        ...
        <tbody>
          <tr v-for="(item,index) in list"  :key="'mater_'+index" @click="curIndex = index">
            ...
            <td> 
              <InputNumber :max="999999" :min="0" v-model="item.packageCount" @on-change="computedAmount(item)"></InputNumber>
            </td>
            <td>
              <Input type="text" v-model="item.vehicleNo" />
            </td>
            <td> 
              <InputNumber  :max="999999" :min="0" v-model="item.roughWeight" @on-change="computedQuantity(item)"></InputNumber>
            </td>
            <td> 
              <InputNumber  :max="999999" :min="0" v-model="item.takeOutWeight" @on-change="computedQuantity(item)"></InputNumber>
            </td>
            <td> 
              <InputNumber  :max="999999" :min="0" v-model="item.packageWeight" @on-change="computedQuantity(item)"></InputNumber>
            </td> 
  ...
  export default {
    ...
    methods: { 
      ...
      computedQuantity(item){
        if(item.roughWeight>0){
          item.quantity = floatObj.subtract(item.roughWeight,item.takeOutWeight);
          item.quantity = floatObj.subtract(item.quantity,item.packageWeight);
          this.computedAmount(item);
        }
      },
      selMater(row){
