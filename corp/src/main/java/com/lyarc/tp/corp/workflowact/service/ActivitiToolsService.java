...
public class ActivitiToolsService {
   ...
   /**
    * 设置会签节点属性 会签相关变量注释：nrOfInstances：实例总数
    * nrOfActiveInstances：当前活动的，比如，还没完成的，实例数量。 对于顺序执行的多实例，值一直为1
    * nrOfCompletedInstances：已经完成实例的数目 可以通过execution.getVariable(x)方法获得这些变量
    *
    * @param modelId
    *            模型id
    * @param nodelId
...
//	}
   ...
   public void setSequenceFlowCondition(String modelId, String nodelId, String condition) throws IOException {
       ...
       if (!(flowElement instanceof SequenceFlow)) {
           throw new CorpException(ResultVo.failure("不是连线，不能设置条件"));
       }
   ...
   public List<PvmActivity> getNextActNodes(String defid, String nodelId, Map<String, Object> elMap) {
       ...
       for (ActivityImpl activityImpl : activitiList) {
           ...
           if (StringUtils.isEmpty(nodelId)) {
               ...
               if (o.equals("startEvent")) {
                   ...
                   //modify by wangting  首节点 并没有流程变量 故使用startevent 的流程变量
                   elMap = activityImpl.getVariables();
                   //modify by wangting getDestination() 已经是下个节点了  再getOutgoingTransitions 相当于下下个节点
//					List<PvmTransition> pvmTransitions = startEvent.getDestination().getOutgoingTransitions();
                   PvmActivity destination = startEvent.getDestination();
                   pvmActivities.add(destination);
//					List<PvmTransition> pvmTransitions = startEvent.getOutgoingTransitions();
//					pvmActivities = getNextActivities(pvmTransitions, elMap);
   ...
   public ProcessInstance findProcessInstanceByTaskId(String taskId) throws Exception {
       ...
       if (processInstance == null) {
           throw new CorpException(ResultVo.failure("流程实例未找到!"));
       }
   ...
   private TaskEntity findTaskById(String taskId) throws Exception {
       ...
       if (task == null) {
           HistoricTaskInstanceEntity hisTask = (HistoricTaskInstanceEntity)historyService.createHistoricTaskInstanceQuery().taskId(taskId).singleResult();
           if (hisTask == null) {
               throw new CorpException(ResultVo.failure("任务实例未找到!"));
           }else{
               task = new TaskEntity();
               task.setProcessInstanceId(hisTask.getProcessInstanceId());
               task.setProcessDefinitionId(hisTask.getProcessDefinitionId());
   ...
   public List<Task> findTaskListByKey(String processInstanceId, String key) {
       List<Task> list = taskService.createTaskQuery().processInstanceId(processInstanceId).taskDefinitionKey(key)
               .list();
       return list;
   }

   /**
    * 根据taskid查询所有需要回退的任务集合
    * 找到当前节点前一个node 后面所有活动，当前流程实例下所有的任务，在前一个节点后面的任务都要complate，分支流程一个在第一个，一个都已经走到后面了
    * 不能回退
    * a-b a-c c-d c-e
    * 当前节点在b,d e  在b退回前一节点，需要把d e 完成
    * @param taskId
    * @return
    */
   public List<Task> findTaskListByTaskId(String taskId) throws Exception {
       List<Task> resList = new ArrayList<>();
       //获取当前节点的source
       ActivityImpl activitiImpl = findActivitiImpl(taskId,"");

       //需要递归获取当前节点前面一个节点后面所有的activitiid

       List<PvmTransition> incomingTransitions = activitiImpl.getIncomingTransitions();
       List<PvmActivity> activitiList = new ArrayList<>();
       List<PvmTransition> outTransList = new ArrayList<>();
       incomingTransitions.forEach(incoming ->{
           List<PvmTransition> outgoingTransitions = incoming.getSource().getOutgoingTransitions();
           outTransList.addAll(outgoingTransitions);
       });

       getAllChildActiviti(outTransList,activitiList);
       ...
       List<Task> list = taskService.createTaskQuery()
               ...
               .list();
       //在这几个任务中查找和taskid 源相同的task
       list.forEach(task->{
           String tId = task.getId();

           ActivityImpl actImpl = null;
           try {
               actImpl = findActivitiImpl(tId,"");
           } catch (Exception e) {
               e.printStackTrace();
           }
           for (PvmActivity pvmActivity : activitiList) {
               if(actImpl.getId().equals(pvmActivity.getId())){
                   resList.add(task);
               }
           }
       });
   ...
   private void getAllChildActiviti(List<PvmTransition> outgoingTransitions,List<PvmActivity> activitiList){
       for (PvmTransition transition : outgoingTransitions) {
           PvmActivity destination = transition.getDestination();

           if(destination != null){
               if(!activitiList.contains(destination)){
                   activitiList.add(destination);
               }
               List<PvmTransition> outgoingTransitions1 = destination.getOutgoingTransitions();
               if(outgoingTransitions1 != null){
                   getAllChildActiviti(outgoingTransitions1,activitiList);
               }
           }
       }
   }
    ...
    * @return
    */
   public ActivityImpl getCurrenActivitiy(String processDefinitionKey,String defId, String activitiId) {
       ProcessDefinitionEntity def = (ProcessDefinitionEntity) ((RepositoryServiceImpl) repositoryService)
               .getDeployedProcessDefinition(defId);
       List<ActivityImpl> activitiList = def.getActivities();
    ...
    * @throws Exception
    */
   public void backProcess(String taskId, String activityId, Map<String, Object> variables, String comment)
           throws Exception {
       Map<String, String> map = new HashMap<>();
       if (org.springframework.util.StringUtils.isEmpty(activityId)) {
           throw new CorpException(ResultVo.failure("驳回目标节点ID为空！"));
       }
       if (org.springframework.util.StringUtils.isEmpty(taskId)) {
           throw new CorpException(ResultVo.failure("任务iD为空！"));
       }
       // 根据任务ID获取对应的流程实例
       ProcessInstance processInstance = findProcessInstanceByTaskId(taskId);
       // 根据任务ID获得任务实例
    ...
    * @return
    */
   private List<PvmTransition> forCheckQueryPvm(List<PvmTransition> pvmTransitions,
       ...
       for (PvmTransition pvmTransition : pvmTransitions) {
           ...
           HistoricTaskInstance historicTaskInstance = historyService.createHistoricTaskInstanceQuery()
                   .taskDefinitionKey(activitid).processDefinitionId(processDef).orderByTaskCreateTime().desc().list()
                   .get(0);
    ...
    * @throws Exception
    */
   private void startParallelGateways(Map map, ProcessInstance processInstance, Map<String, Object> variables,
    ...
    * @throws Exception
    */
   private void endParallelGateways(Map map, ProcessInstance processInstance, Map<String, Object> variables,
           List<String> list, String comment) throws Exception {
    ...
    * @throws Exception
    */
   private void comment(Map map, ProcessInstance processInstance, Map<String, Object> variables,
           String activityId, String comment) throws Exception {
       List<Task> taskList = new ArrayList<>();
       Iterator it = map.keySet().iterator();
       while (it.hasNext()) {
    ...
    * @throws Exception
    */
   public void commitProcess(String taskId, Map<String, Object> variables, String activityId, String comment)
           throws Exception {
       if (variables == null) {
    ...
    * @throws Exception
    */
   private void commitProcess(String taskId, Map<String, Object> variables, List<String> activityIds,
           String comment) throws Exception {
       if (variables == null) {
           variables = new HashMap<>();
       }
       // 跳转节点为空，默认提交操作
       if (org.springframework.util.StringUtils.isEmpty(activityIds)) {
           taskService.complete(taskId, variables);
       } else {// 流程转向操作
    ...
    * @throws Exception
    */
   public void turnTransition(String taskId, List<String> activityIds, Map<String, Object> variables,
           String comment) throws Exception {
       // 当前节点
       ActivityImpl currActivity = findActivitiImpl(taskId, null);
       // 清空当前流向
       ...
       List<ActivityImpl> pointActivitys = new ArrayList<>();
    ...
    * @throws Exception
    */
   public void turnTransition(String taskId, String activityId, Map<String, Object> variables, String comment)
           throws Exception {
       // 当前节点
       ActivityImpl currActivity = findActivitiImpl(taskId, null);
       // 清空当前流向
     ...
     * @throws Exception
     */
    public void recallTransition(String taskId, String activityId, Map<String, Object> variables, String comment)
            throws Exception {
        // 当前节点
        ActivityImpl currActivity = findActivitiImpl(taskId, null);
        // 清空当前流向
    ...
    /**
     *
     * @description 只是完成当前任务，并不流转到下一节点 用于并行任务回退
     * @param
     * @return
     * @author wangting
     * @date 2018/5/4 0004 17:57
     *
     */
   ...
   public void setBackTaskDealer(Task task, String activityId) {
       List<HistoricTaskInstance> list = historyService.createHistoricTaskInstanceQuery()
               .processInstanceId(task.getProcessInstanceId()).taskDefinitionKey(activityId)
               .taskDeleteReason("completed").orderByTaskCreateTime().desc().list();
       ...
       if (list != null && list.size() > 0) {
           ...
           if (taskList != null && taskList.size() > 1) {
               ...
               if (!org.springframework.util.StringUtils.isEmpty(variable)) {
                   // 设置下个顺序会签处理人
   ...
   /**
    * (取上一个任务节点) 根据节点Id取得当前节点的上一流向,分支路由结束点，需要查询历史任务出现过的分支结点，并行路由则随便取一个
    *
    * @param processDefinitionKey
    *            流程定义Key
    *@param defId
    ...
    * @return
    */
   public String getProActivityId(String processDefinitionKey,String defId, String activitiId, String processInstanceId,
           String preNodeId) {
       //并行路由前面会有多个activiti
...
//				.processDefinitionId(defId).singleResult();
       ...
       for (ActivityImpl activityImpl : activitiList) {
           String id = activityImpl.getId();
           if (!org.springframework.util.StringUtils.isEmpty(activitiId)) {
               if (activitiId.equals(id)) {
                   ...
                   for (PvmTransition tr : pvmTransitions) {
                       ...
                       if ("userTask".equals(type)) {
                           preNodeId = ac.getId();
                           //并行路由前面有多个userTask
                           sb.append(preNodeId).append(",");
//							return preNodeId;
                       } else if ("exclusiveGateway".equals(type)) {
                           PvmActivity exclusiveActiviti = getExclusiveActiviti(ac, processDefinitionKey,defId,
                                   processInstanceId);
                           ...
                           throw new CorpException(ResultVo.failure("首节点无法回退"));
                       }else {
                           // 其它情况则进行递归 并行路由
                           preNodeId = getProActivityId(processDefinitionKey,defId, ac.getId(), processInstanceId,
                                   preNodeId);
                           sb.append(preNodeId).append(",");
                       }
                   }
    ...
    * @return
    */
   public PvmActivity getExclusiveActiviti(PvmActivity ac, String processDefinitionKey,String defId,
       ...
       for (PvmTransition p : exclusiveEnds) {
           ...
           HistoricTaskInstance taskInstance = historyService.createHistoricTaskInstanceQuery()
                   ...
                   .taskDefinitionKey(pvmActivity.getId()).orderByTaskCreateTime().desc().list().get(0);
           // 有记录
           if (taskInstance != null && !org.springframework.util.StringUtils.isEmpty(taskInstance.getId())) {
       ...
       return getCurrenActivitiy(processDefinitionKey,defId, activityId);
   }
   ...
   public boolean transferAssignee(String taskId, String userId) {
     ...
     * @return
     * @author wangting
     * @date 2018/5/28 0028 15:33
     *
     */
    public void endProcess(String taskId,String instanceId,String comment) throws Exception {
        ActivityImpl endActivity = findActivitiImpl(taskId, "end");
        commitProcess(taskId, null, endActivity.getId(),comment);

        //修改历史任务及实例delete_reason 为终止
        executeCommand(new UpdateHiTaskReasonCommand(taskId, WorkFlowConst.TASK_REASON_TERMINATION));
        executeCommand(new UpdateHiInstanceReasonCommand(instanceId, WorkFlowConst.TASK_REASON_TERMINATION));
    }

    public void executeCommand(Command<Void> command)
    {
        ((HistoryServiceImpl)historyService).getCommandExecutor().execute(command);
    }
}
