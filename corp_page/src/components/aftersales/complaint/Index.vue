<template>
  <ListPage ref="page" title="售后投诉服务管理" api="/api/complaint/list" 
...
<script>
  ...
  export default {
          ...
          },
          {
            ...
            render:(h,params)=>{
              ...
              return h('a',{
                ...
                on:{
                  click:()=>{
                    this.$router.push({path:'/complaint/view?forward&id='+row.complaintId});
                  }
                }
              },text);
            }
            ...
            render:(h,params)=>{
              ...
              if(row.disposeStatus>0){
                return h('a',{
                  ...
                  on:{
                    click:()=>{
                      this.$router.push({path:'/disposition/view?forward&id='+row.disposeId});
                    }
                  }
                },text);
                ...
                return h('a',{
                  ...
                  on:{
                    click:()=>{
                      this.$router.push({path:'/disposition/edit?forward&complaintId=' + row.complaintId})
                    }
                  }
                },'创建处置单');
              }else{
    ...
    methods: {
      ...
      add() { 
        this.$router.push({path:'/complaint/edit?forward'})
      },
      del(){
          this.$Modal.confirm({
          title: '删除确认',
          ...
          onOk: () => {
            ...
            if(this.curRow){
            this.loading = 1;
            this.$http.post('/api/complaint/delete',{
              complaintId:this.curRow.complaintId,
            }).then((res) => {
              this.loading = 0;
              if (res.data.code === 0) {
