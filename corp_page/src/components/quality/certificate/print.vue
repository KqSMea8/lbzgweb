<template>
    <ListPage ref="page" title="合格证打印" :api="api" :queryForm="queryForm" :list="list"
        ...
        <div>
                    ...
                    <tr>
                        ...
                        <td>
                            <DatePicker type="daterange" :start-date="new Date(2018,1,1)" :end-date="new Date()" placement="bottom-end"
                                        placeholder="开始日期" v-model="createTime" style="width: 200px"></DatePicker>
                        </td>
            ...
            <div>
                <table cellpadding="5" class="hgz" cellspacing="0"
                       style="text-align: left;margin: 10px auto;overflow:auto; ">
                    <thead>
                    <tr>
                        <th>外销</th>
                        <th>合同编号</th>
                    ...
                    <template v-for="(item,index) in list">
                        ...
                        <template v-for="(item2,index2) in item.maps">
                            <tr v-for="(item3,index3) in item2.castingCertificate.testItemObj">
                                <td :rowspan="item.rowIndex" v-show="ss==0"><span
                                        v-if="item.isForeign==1">&#10004;</span></td>
                                ...
                                <td class="hgz_tr" :rowspan="item2.castingCertificate.testItemObj.length"
                                    v-show="index3==0">
                                    <i-button type="info" size="small" @click="casting_open(index,index2,index3)"
                                              v-show="item2.castingCertificate.certificateId!=null">铸件合格证
                                    </i-button>
                                    <i-button type="info" size="small" @click="ultrasonic_open(index,index2,index3)"
                                              v-show="item2.magneticPowderInspection.magneticId!=null">超声波探伤检验报告
                                    </i-button>
                                    <i-button type="info" size="small" @click="magnetic_open(index,index2,index3)"
                                              v-show="item2.ultrasonicTesting.ultrasonicId!=null">磁粉探伤检验报告
                                    </i-button>
                                    <i-button type="info" size="small" @click="size_open(index,index2,index3)"
                                              v-show="item2.partSize.partSizeId!=null">零件加工尺寸报告
                                    </i-button>
                                </td>
                            </tr>
                        </template>
