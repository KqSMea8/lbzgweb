  ...
  <div class="page page-bill">
    <div class="page-bar">
      <LayoutHor>
        ...
        <div class="page-bar-title">合同预算重量单 - 处理</div>
      </LayoutHor>
    </div>
      ...
      <Row>
        ...
        <Col span="10">
          <table>
            ...
            <tr>
              <td><UploadBox v-model="formItem.files" :readonly="true"
                               style="height: 90px;"></UploadBox></td>
            </tr>
          </table>
        </Col>
      </Row>
...
<script>
  ...
  export default {
    ...
    methods: {
      initNew(){
        this.formItem.completeTime = dateFormat(new Date(),'yyyy-MM-dd');
        if(this.$user.empInfo){
          this.formItem.designer = this.$user.empInfo.empId;
          this.formItem.designerName = this.$user.empInfo.trueName;
        }
      },
      load() {
        this.loading = 1;
      ...
      save(){
        ...
        this.loading = 1;
        this.$http.post("/api/inquire/weight/submit",this.oriItem).then((res) => {
          this.loading = 0;
          if (res.data.code == 0) {
