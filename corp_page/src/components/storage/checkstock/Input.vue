<template>
  ...
  :model="this" class="customerlist" @on-load-data="loadData">
    <div class="page-title">
      <a @click="goPage('/storage/mgr')">仓库管理</a> &gt; 盘点 - 录入
    </div>
    <div class="page-tools">
      <table cellpadding="0" cellspacing="0">
        <tr> 
          ...
          <td><Button @click="checkSubmit" v-power="'ckgl.pd.submit'" icon="checkmark">提交审核</Button></td>
        </tr>
      </table>
    </div>
  ...
  export default {
    ...
    methods: {
      ...
      save(){
        ...
        this.$http.post('/api/storage/stock/check/batch', data)
        .then((res) => {
          if (res.data.code === 0) { 
      ...
      checkSubmit(){
        ...
        this.$Modal.confirm({
            ...
            onOk: () => {
              this.$http.post('/api/storage/stock/check/submit?storageId=' + this.queryForm.storageId, {}).then((res) => {
                if (res.data.code === 0) {
                  this.$Message.success("操作成功！");
                  this.query();
                } else {
